...
- **Full metro, real heights** — every building in the metro bbox (`36.02,-96.15 → 36.25,-95.75`). Footprints are a hybrid: **OSM ways** (72.9k, architectural outlines, best downtown) merged with **Microsoft ML Building Footprints** (145k gap-fill where OSM has no coverage). Heights resolve in priority order: **USGS 3DEP lidar measurements** (45.8k buildings in the inner ±6 km — see below), OSM `height`/`building:levels` tags where surveyed, Microsoft's ML height estimates transferred onto untagged OSM footprints, and seeded residential defaults for the rest.
- **Lidar-measured heights** — `scripts/bake-lidar.mjs` streams ~335M points from the USGS 3DEP Entwine archive (`USGS_LPC_OK_Woodward_UTM15_B6_2016`, the QL2 collection covering Tulsa) and rasterizes a 2 m DSM + ground grid over the inner city. The metro bake then measures each footprint: median for houses (damps tree overhang), highest well-supported roof plateau for large block polygons (so a plaza doesn't inherit its tower's height, and antenna spikes are rejected). Downtown heights are now survey-grade.
...
- **Orthophoto rooftops & photo-sampled colors** — flat roof caps are UV-mapped straight onto the USGS/NAIP aerial photo (tiered: ~1.1 m/px downtown core, ~3 m/px inset, ~9 m/px metro), so every rooftop is the real Tulsa rooftop — HVAC clusters, parking decks, roof colors. Wall albedo pulls toward the photographed roof color read from a sampling canvas: brick reads brick, white stone reads white.
- **Imagery-seeded tree canopy** — 30k instanced low-poly crowns are planted wherever the aerial photo pixel is vegetation-green (inset ±6 km), each colored from its own photo pixel. Tulsa's neighborhood canopy emerges from the imagery itself; hidden in 1943 and photoreal modes.
...
- **Heights**: lidar-measured inside the ±6 km inner city (2016 acquisition — buildings newer than that read short); outside it, Microsoft ML estimates (tend to underestimate towers) and OSM tags. Buildings under masts/water towers can still read tall despite plateau filtering.
